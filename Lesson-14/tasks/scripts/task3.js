if (confirm('Почати тестування?')) {
  ...
  function getTotalPrice(arr) {
    return arr.reduce((prevSum, el) => prevSum + el.price, 0)
  }
  //  Знайти кількість товарів, у яких ціна зменшилась(price < old_price).
  function getCheaperGoods(arr) {
    return arr.reduce((acc, el) => el.price < el.old_price ? acc + 1 : acc, 0)
  }
  // Товари, які доступні(sell_status: "available")
  function getAvailableGoods(arr) {
    return arr.filter(el => el.sell_status === 'available')
  ...
  function getNewArray(arr) {
    return arr.reduce((acc, el) => {
      if (el.sell_status === 'available')
        acc.push({ id: el.id, price: el.price, old_price: el.old_price, usd_price: el.usd_price })

      return acc
    }, [])
  }

  document.write(`<div>Загальна вартість товарів ${getTotalPrice(dataList)} грн.</div>`)
  document.write(`<div>Ціна зменшилась у ${getCheaperGoods(dataList)} товарів.</div>`)
  console.log('Товари, які доступні')
  console.log(getAvailableGoods(dataList))
  console.log('Товари, які доступні (новий список)')
  console.log(getNewArray(dataList))
}